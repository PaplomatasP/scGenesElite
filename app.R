...
server <- function(input, output, session) {
  ...
  source('./Scripts/ShapValuesFilter.R', local = TRUE)
  source('./Scripts/SimpleShapFilter.R', local = TRUE)
  source('./Scripts/RobustShapFilter.R', local = TRUE)
  ...
  source('Scripts/UploadPreview.R', local = TRUE)
      ...
      withProgress(message = 'Please wait........', value = 0, {
        ...
        if (websiteLive) {
          ...
          for (i in seq_along(enriched)) {
            ...
            # GC Petros S 12: Improve Enrichment Analysis plot - labels inside bars
            # Truncate long pathway names for better display
            allPlotData$Term_Short <- ifelse(
              nchar(allPlotData$Term) > 50,
              paste0(substr(allPlotData$Term, 1, 47), "..."),
              allPlotData$Term
            )
            
            BBP[[i]] <-
              ggplot(data = allPlotData, aes(
                x = reorder(Term_Short, +Combined.Score),
                y = Combined.Score
              )) +
              ggtitle(dbs[i]) +
              geom_bar(stat = "identity",
                       position = "dodge",
                       aes(fill = Combined.Score)) + 
              coord_flip() + 
              theme_gray() +
              theme(
                ...
                plot.margin = ggplot2::margin(l = 120, r = 20, t = 10, b = 10, unit = "pt")  # Add left margin for labels
              ) +
              # Add text on the left side (at the start of plot) for better visibility
              # This ensures labels are always visible, even for small bars
              # Using x = Term_Short and y = 0 positions text at the start (left side after coord_flip)
              geom_text(
                aes(label = Term_Short, x = reorder(Term_Short, +Combined.Score), y = 0),
                hjust = 0,         # Left-align text (starts from left edge)
                size = 3.2,        # Readable text size
              ...
              scale_fill_distiller(name = "Value",
                                   palette = "Reds",
                                   direction = 1) +
              scale_y_continuous(expand = expansion(mult = c(0, 0.1)))  # Add space on right for labels
