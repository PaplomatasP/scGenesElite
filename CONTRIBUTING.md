...
Use the [bug report form](https://github.com/PaplomatasP/scGenesElite/issues/new?template=bug_report.yml). Include the method and settings, expected behavior, actual behavior, console error and `sessionInfo()`. A small example that reproduces the problem is more useful than an image of the error alone.

For a proposed change, describe the analysis task it supports and how its behavior could be checked. Use the [feature request form](https://github.com/PaplomatasP/scGenesElite/issues/new?template=feature_request.yml).
